...
class SnakeGameAI:
    ...
    def __init__(self, affichage=True, limite_etapes=True, facteur_vitesse=1):
        ...
        if self.affichage:
            ...
            titre = "Snake IA - Deep Q-Learning"
...
LR = 0.001
GAMMA = 0.9
# État étendu (18 valeurs) : ajoute « queue atteignable » pour chaque action et la longueur du serpent.
# Au chargement d'un modèle, la taille de l'état est détectée automatiquement depuis le fichier.
ETAT_ETENDU = False
# Pendant l'entraînement, le modèle est évalué toutes les EVAL_TOUS parties sur EVAL_PARTIES parties
# fixes (sans hasard) : on garde le meilleur, ce qui protège contre l'« oubli » du réseau.
EVAL_TOUS = 50
EVAL_PARTIES = 30
...
class Agent:
    ...
    def __init__(self, etat_etendu=None):
        self.n_games = 0
        self.memory = deque(maxlen=MAX_MEMOIRE)  # les plus anciens souvenirs sont oubliés
        self.etat_etendu = ETAT_ETENDU if etat_etendu is None else etat_etendu
        self.model = Linear_QNet(self.taille_etat(), 256, 3)
        self.trainer = QTrainer(self.model, lr=LR, gamma=GAMMA)

    def taille_etat(self):
        return 18 if self.etat_etendu else 14

    def charger(self, dossier=DOSSIER_MODELE):
        """Charge model.pth en adaptant la taille de l'état à celle du modèle enregistré."""
        poids = torch.load(os.path.join(dossier, "model.pth"))
        self.etat_etendu = poids["linear1.weight"].shape[1] == 18
        self.model = Linear_QNet(self.taille_etat(), 256, 3)
        self.model.load(dossier=dossier)

    def get_state(self, game):
        """État à 14 valeurs : 3 dangers, 4 directions, 4 positions de pomme, 3 espaces libres.
        État étendu (18 valeurs) : + 3 « queue atteignable » + longueur du serpent."""
        snake = game.snake
        ...
        def espace_libre(d):
            ...
            # queue atteignable : suivre sa queue permet de survivre indéfiniment
            return min(1.0, (len(vus) - 1) / max(1, cases_libres - 1)), queue in vus
        ...
        espaces = [espace_libre(direction), espace_libre(dir_droite), espace_libre(dir_gauche)]
        state += [espace for espace, _ in espaces]  # espace libre tout droit / à droite / à gauche
        if self.etat_etendu:
            state += [queue_ok for _, queue_ok in espaces]  # queue atteignable tout droit / droite / gauche
            state.append(longueur / (g * g))                # longueur du serpent (0 à 1)
...
def train(nb_parties=300, dossier=DOSSIER_MODELE, seed=None, eval_tous=EVAL_TOUS, eval_parties=EVAL_PARTIES):
    ...
    meilleure_eval = None  # (score moyen, -temps moyen) du meilleur modèle sauvegardé
    complet = True
    debut = time.time()
        ...
        try:
            ...
            while agent.n_games < nb_parties:
                ...
                if done:
                    ...
                    if eval_tous and agent.n_games % eval_tous == 0:
                        res = jouer_parties_eval(agent, eval_parties, seed=12345)
                        moy_score = sum(r[0] for r in res) / len(res)
                        moy_temps = sum(r[1] for r in res) / len(res)
                        note = (moy_score, -moy_temps)
                        garde = meilleure_eval is None or note > meilleure_eval
                        if garde:
                            meilleure_eval = note
                            agent.model.save(dossier=dossier)
                        print(f"  Évaluation après {agent.n_games} parties : moyenne {moy_score:.1f}, "
                              f"max {max(r[0] for r in res)}" + (" -> modèle sauvegardé" if garde else ""))
    ...
    if eval_tous and meilleure_eval is None:  # interrompu avant la première évaluation
        agent.model.save(dossier=dossier)
    print(f"Terminé : record d'entraînement {record}"
          + (f", meilleur modèle évalué : moyenne {meilleure_eval[0]:.1f}" if meilleure_eval else "")
          + f", modèle dans {dossier}, historique dans {chemin_csv}")
    return complet
...
def demo(dossier=DOSSIER_MODELE, facteur_vitesse=1):
    ...
    print(f"  Modèle : {os.path.join(dossier, 'model.pth')} ({agent.taille_etat()} entrées)")
...
def jouer_parties_eval(agent, nb_parties, seed=None):
    """Joue nb_parties comme en partie réelle (sans hasard de l'agent, sans limite), sans affichage.
    Retourne [(score, temps de jeu en s, fin de partie)]. Temps de jeu = coups / GAME_SPEED.
    L'état du générateur aléatoire est restauré ensuite pour ne pas perturber l'entraînement."""
    etat_aleatoire = random.getstate()
    if seed is not None:
        random.seed(seed)
    ...
    random.setstate(etat_aleatoire)
    return resultats
...
def evaluate(nb_parties=100, dossier=DOSSIER_MODELE, seed=None):
    ...
    agent = Agent()
    agent.charger(dossier)
    resultats = jouer_parties_eval(agent, nb_parties, seed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Snake - Deep Q-Learning")
    ...
    p_train.add_argument("--etendu", action="store_true", help="état à 18 valeurs (queue atteignable + longueur)")
